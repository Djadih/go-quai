...
ifeq (,$(wildcard nodelogs))
	...
	@nohup $(BASE_CMD) --miner.etherbase $(ZONE_0_0_COINBASE) --port $(ZONE_0_0_PORT_TCP) --http.port $(ZONE_0_0_PORT_HTTP) --ws.port $(ZONE_0_0_PORT_WS) --dom.url $(ZONE_0_0_DOM_URL):$(REGION_0_PORT_WS)  --pprof --metrics --metrics.influxdb --metrics.influxdb.endpoint "http://0.0.0.0:8086" --metrics.influxdb.username "user" --metrics.influxdb.password "password" --metrics.influxdb.database "quai" --region 0 --zone 0 >> nodelogs/zone-0-0.log 2>&1 &
