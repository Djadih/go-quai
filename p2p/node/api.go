...
func (p *P2PNode) RequestBlock(hash types.Hash, slice types.SliceID) chan *types.Block {
	...
	go func() {
		...
		// 2. If not, query the local routing table for the peer that should have the block
		for _, peer := range p.routingTables[slice] {
			// query the peer for the block
			block, err := p.requestBlockFromPeer(hash, peer.ID)
			if err != nil {
				// if the peer responds with an error, remove the peer from the routing table
				// TODO: implement
				continue
			}
			log.Debugf("Received block %s from peer %s", block.Hash, peer.ID)
