...
type GenesisAccount struct {
	...
	TotalBalance    *big.Int                                 `json:"Total"`
...
func CalculateGenesisUnlocks(filename string) ([]GenesisAccount, error) {
...
// Parses the allocs JSON and populates unlock info.
...
func (account *GenesisAccount) calculateLockedBalances() {
	...
	lumpSumAmount := lumpSumPercentage.Mul(account.VestedBalance, lumpSumPercentage) // for testing
	...
	if unlockSchedule.unlockDuration != 0 {
		...
		unlockableBalance := new(big.Int).Sub(account.TotalBalance, lumpSumAmount)
		...
		remainingBalance := new(big.Int).Sub(account.VestedBalance, lumpSumAmount)
