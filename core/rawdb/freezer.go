...
func (f *freezer) AppendAncient(number uint64, hash, header, body, receipts, etxSet []byte) (err error) {
	...
	defer func() {
		if err != nil {
			rerr := f.repair()
			if rerr != nil {
				log.Error("Failed to repair freezer", "err", rerr)
			}
...
func (f *freezer) freeze(db ethdb.KeyValueStore) {
	...
	for {
		...
		if err := batch.Write(); err != nil {
			log.Error("Failed to delete frozen canonical blocks", "err", err)
		}
		batch.Reset()
		...
		if err := batch.Write(); err != nil {
			log.Error("Failed to delete frozen side blocks", "err", err)
		}
		batch.Reset()
		...
		if f.frozen > 0 {
			...
			if err := batch.Write(); err != nil {
				log.Error("Failed to delete dangling side blocks", "err", err)
			}
		}
