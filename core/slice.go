...
func makeDomClient(domurl string) *quaiclient.Client {
	if domurl == "" {
		log.Error("dom client url is empty")
	}
	domClient, err := quaiclient.Dial(domurl)
	if err != nil {
		log.Error("Error connecting to the dominant go-quai client", "err", err)
	}
	return domClient
}
...
func makeSubClients(suburls []string) []*quaiclient.Client {
	...
	for i, suburl := range suburls {
		if suburl != "" {
			...
			if err != nil {
				log.Error("Error connecting to the subordinate go-quai client for index", "index", i, " err ", err)
			}
