...
type ApiOperation[T Number | string | bool] func(actual string) (T, bool, error)

func WithRequire[T Number | string | bool](parse ParseString[T]) ApiOperation[T] {
	var empty T
...
func WithDefaultOrParse[T Number | string | bool](def T, parse ParseString[T]) ApiOperation[T] {
	return func(actual string) (T, bool, error) {
		if actual == "" {
			return def, true, nil
		}
...
func WithParse[T Number | string | bool](parse ParseString[T]) ApiOperation[T] {
	return func(actual string) (T, bool, error) {
		v, err := parse(actual)
		return v, false, err
	}
...
func parseNumericParameter[T Number](param string, fn ApiOperation[T], checks ...Constraint[T]) (T, error) {
	v, ok, err := fn(param)
	if err != nil {
		return 0, err
	}
...
func parseBoolParameter(param string, fn ApiOperation[bool]) (bool, error) {
	v, _, err := fn(param)
	return v, err
}

// parseNumericArrayParameter parses a string parameter containing array of values to its respective type.
func parseNumericArrayParameter[T Number](param, delim string, required bool, fn ApiOperation[T], checks ...Constraint[T]) ([]T, error) {
	if param == "" {
		if required {
